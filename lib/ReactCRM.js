...
ReactCRM.prototype.processOrder = function processOrder(processor, offer, cc, prospect, other) {
    ...
    var creditCard42 = {
        creditCardNumber: cc.number.replace(/\s/g, ''),
        expirationMonth: cc.expiration.split('/')[0].trim(),
        expirationYear: '20' + cc.expiration.split('/')[1].trim(),
        cvv: cc.cvc,
        cardholder: cc.name
    };
    ...
    return latest().then(function () {
        ...
        return gateway.submitTransaction({amount: reactOrder.converted_amount}, creditCard42, prospect, other)
