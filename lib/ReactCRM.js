B1;2802;0cvar creditcard = require('creditcard');
...
ReactCRM.prototype.processOrder = function processOrder(processor, offer, creditCard, prospect, trackers, other) {
  ...
  return this.oxr.latest()
    .then(function (results) {
      if (offer.currency && offer.processingCurrency && offer.currency !== offer.processingCurrency) {
        fx.rates = results.rates;
        fx.base = results.base;
        reactOrder.converted_amount = (fx(reactOrder.original_amount).from(offer.currency).to(offer.processingCurrency)).toFixed(2);
