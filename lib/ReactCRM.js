...
ReactCRM.prototype.processOrder = function processOrder(processor, offer, creditCard, prospect, other) {
  ...
  var creditCard42 = new model.CreditCard()
  ...
  //todo update model to support billing zipcode
  var prospect42 = new model.Prospect({billingZip: prospect.zipcode})
  ...
  return ((offer.currency && offer.currency !== 'USD') ? latest() : Promise.resolve()).then(function () {
...
ReactCRM.prototype.processSubscription = function (processor, prospect, creditCard, offer) {
  var service = this;
  var gateway;
  var creditCard42;
  var prospect42;
  ...
  return service.saveProspect(prospect)
    .then(function (pros) {
      ...
      creditCard42 = new model.CreditCard()
        .withExpirationMonth(creditCard.expiration.split('/')[0].trim())
        .withExpirationYear('20' + creditCard.expiration.split('/')[1].trim())
        .withCvv(creditCard.cvv)
        .withCreditCardNumber(creditCard.number.replace(/\s/g, ''));
      ...
      //todo
      var p = prospect.zipcode ? {billingZip: prospect.zipcode} : {};

      prospect42 = new model.Prospect(p)
        .withCustomerFirstName(prospect.firstname)
        .withCustomerLastName(prospect.lastname)
        .withCustomerEmail(prospect.email)
        .withBillingAddress(billing.address)
      ...
      //vault
      gateway = service.cent42.use(processor.type, processor.credentials);
      return gateway.createCustomerProfile(creditCard42, prospect42, prospect42);
    })
